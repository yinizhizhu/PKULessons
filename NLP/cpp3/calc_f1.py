...
if (__name__ == '__main__'):
    ...
    outFile = 'result.txt'
    if sys.argv[1] == '1':
        outFile = 'devLabel/'+outFile
        ans = calc_f1('data/cpbdev.txt',
                  'devLabel/demoFeatureDevLabelSentence_'+sys.argv[2]+'_'+sys.argv[3]+'.txt')
    else:
        outFile = 'final/'+outFile
        ans = calc_f1('data/answer.txt',
                  'final/testLabelSentence_'+sys.argv[2]+'_'+sys.argv[3]+'.txt')
    out = open(outFile, 'a')
    if sys.argv[6] == '0':
        print >> out, ''
    print >> out, sys.argv[4], '*', sys.argv[5], ': ',
    print >> out, ans
    out.close()
